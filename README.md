...
6. **复读指令 `转语音 <内容>`**（v1.0.7 起，别名 `转语言`；v1.0.9 加 `prefix_optional`）：机器人一字不差地念出你输入的内容（不经过 LLM），同时发送语音消息和 wav 文件。**不需要 `/` 前缀**，直接发 `转语音 今天天气真不错` 即可（也支持 `@机器人 转语音 内容`）。权限由 `custom` 组控制：`say_admin_only`（默认开，仅群管理/AstrBot 管理员）、`say_whitelist`（额外放行的 QQ 号）、`say_send_wav_file`（是否附带 wav 文件，默认开）；
7. **状态提示**（v1.0.8）：`/转语音` 触发后立即回复"正在转化…"提示（长文本提示分段合成），合成成功再发语音+wav；失败/超时给出原因和建议。利用 handler 多次 yield 实现逐条推送。
